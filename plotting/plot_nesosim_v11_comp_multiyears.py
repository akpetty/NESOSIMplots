...
for x in range(len(years)):
	...
	folderStr1='ERAIsfERAIwindsOSISAFdriftsCDRsicrhovariable_IC2_DYN1_WP1_LL1_WPF5.8e-07_WPT5_LLF2.9e-07-100kmv11oct14'
	totalOutStr1=''+folderStr1+'-'+dateOut

	folderStr2='ERA5sfERAIwindsOSISAFdriftsCDRsicrhovariable_IC2_DYN1_WP1_LL1_WPF5.8e-07_WPT5_LLF2.9e-07-100kmv11oct15'
	totalOutStr2=''+folderStr2+'-'+dateOut

	# take the last day of both!
	snowDepthMv11=ut.get_budgets2layers_day(['snowDepthTotalConc'], model_save_path+dxStr+'/', folderStr1, -1, totalOutStr1, converttocm=1)
	snowDepthMv11[~np.isfinite(snowDepthMv11)]=np.nan

	# take the last day of both!
	snowDepthMv11_2=ut.get_budgets2layers_day(['snowDepthTotalConc'], model_save_path+dxStr+'/', folderStr2, -1, totalOutStr2, converttocm=1)
	snowDepthMv11_2[~np.isfinite(snowDepthMv11_2)]=np.nan
	

	snowDepthMv11_2s.append(snowDepthMv11_2)
	...
	cs=ax1.pcolormesh(lonG, latG, snowDepthMv11, vmin=minval, vmax=maxval, transform=ccrs.PlateCarree(), zorder=2)
	...
	cs2=ax2.pcolormesh(lonG, latG, snowDepthMv11_2, vmin=minval, vmax=maxval, transform=ccrs.PlateCarree(), zorder=2)
	...
	cs3=ax3.pcolormesh(lonG, latG, snowDepthMv11_2-snowDepthMv11, vmin=-20, vmax=20, cmap=plt.cm.RdBu_r, transform=ccrs.PlateCarree(), zorder=2)
	...
	plt.savefig(figure_path+'/model_comps/v11'+dateOut[-8:]+folderStr1+folderStr2+'.png', dpi=300)

snowDepthMv11_mean=np.nanmean(snowDepthMv11s, axis=0)
snowDepthMv11_2_mean=np.nanmean(snowDepthMv11_2s, axis=0)

fig=plt.figure(figsize=(7, 3))
ax1 = plt.subplot(1, 3, 1, projection=ccrs.NorthPolarStereo(central_longitude=-45))
plt.subplots_adjust(bottom=0.13, left=0.02, right=0.98, wspace=0.05)

cs=ax1.pcolormesh(lonG, latG, snowDepthMv11_mean, vmin=minval, vmax=maxval, transform=ccrs.PlateCarree(), zorder=2)
...
ax2 = plt.subplot(1, 3, 2, projection=ccrs.NorthPolarStereo(central_longitude=-45))
cs2=ax2.pcolormesh(lonG, latG, snowDepthMv11_2_mean, vmin=minval, vmax=maxval, transform=ccrs.PlateCarree(), zorder=2)
...
cs3=ax3.pcolormesh(lonG, latG, snowDepthMv11_2_mean-snowDepthMv11_mean, vmin=-20, vmax=20, cmap=plt.cm.RdBu_r, transform=ccrs.PlateCarree(), zorder=2)
...
plt.savefig(figure_path+'/model_comps/v11'+dateOut[-8:-4]+str(years[0])+'-'+str(years[-1])+folderStr1+folderStr2+'.png', dpi=300)
